...
for (K in K_values) {
  for (n in n_values) {
    filename <- sprintf("rdata/gmm%d_2d_multiple_Y_n%d.RData", K, n)
    
    if (file.exists(filename)) {
      load(filename)
      all_data[[paste0("K", K, "_n", n)]] <- list(
        K = K, n = n, results = gmm_2d_multiple_Y_results
      )
    }
  }
}
...
df_combined <- bind_rows(
  lapply(names(all_data), function(key) {
    ...
    bind_rows(
      tibble(method = "THAMES", diff = res$thames_diff, time = res$time_thms, K = K, n = n),
      tibble(method = "MixTHAMES", diff = res$mixturethames_diff, time = res$time_mixthms, K = K, n = n),
      tibble(method = "ECMLE", diff = res$ecmle_diff, time = res$time_ecmle, K = K, n = n),
      tibble(method = "ePWK", diff = res$epwk_diff, time = res$time_pwk, K = K, n = n)
    )
  })
) %>%
...
y_scales <- list(
  ...
  scale_y_continuous(limits = c(0.97, 1.02)),
  scale_y_continuous(limits = c(0.9, 1.2)),
...
p_box <- ggplot(df_combined, aes(x = method, y = ratio, fill = method)) +
  ...
  facet_grid2(K ~ n, scales = "free_y", independent = "y", axes = "y") +
