...
for (n in n_values) {
  filename <- sprintf("rdata/gaussian_2d_multiple_Y_n%d.RData", n)
  
  if (file.exists(filename)) {
    load(filename)
    all_data[[paste0("n", n)]] <- list(n = n, results = gaussian_2d_multiple_Y_results)
  }
}
...
# ============================================================
y_scales <- list(
  scale_y_continuous(limits = c(0.99, 1.04)),
  scale_y_continuous(limits = c(0.99, 1.04)),
  scale_y_continuous(limits = c(0.99, 1.04))
)
